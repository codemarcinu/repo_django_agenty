...
class DashboardPerformanceTest(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            category = Category.objects.create(
                name=f"Category {i}", meta={"default_expiry_days": 7 + (i * 3)}
            )
    ...
    def test_large_dataset_performance(self):
        ...
        print(
            f"Testing with {Product.objects.count()} products, {InventoryItem.objects.count()} inventory items"
        )
...
class InventoryListPerformanceTest(TestCase):
    ...
    def test_inventory_list_with_filters_performance(self):
        ...
        url = reverse("inventory:inventory_list")
        params = {"search": "Product", "location": "pantry", "status": "low_stock"}
