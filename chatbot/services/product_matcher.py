...
class ProductMatcher:
    ...
    def __init__(self):
        ...
        self.category_keywords = {
            'dairy': ['mleko', 'milk', 'jogurt', 'yogurt', 'ser', 'cheese', 'masło', 'butter', 'śmietana', 'cream'],
            'meat': ['mięso', 'meat', 'kiełbasa', 'sausage', 'wędlina', 'szynka', 'ham', 'kurczak', 'chicken'],
            'vegetables': ['warzywa', 'vegetables', 'marchew', 'carrot', 'ziemniak', 'potato', 'cebula', 'onion'],
            'fruits': ['owoce', 'fruits', 'jabłko', 'apple', 'banan', 'banana', 'pomarańcza', 'orange'],
            'bread': ['chleb', 'bread', 'bułka', 'roll', 'bagietka', 'baguette'],
            'beverages': ['napoje', 'beverages', 'woda', 'water', 'sok', 'juice', 'cola', 'piwo', 'beer'],
            'cleaning': ['czyszczenie', 'cleaning', 'detergent', 'mydło', 'soap', 'proszek'],
    ...
    def match_product(self, parsed_product: ParsedProduct) -> MatchResult:
        ...
        if alias_match:
            logger.info(f"Alias match found: {alias_match.name} (alias: {matched_alias})")
            return MatchResult(
                product=alias_match,
                confidence=0.9,
                match_type='alias',
        ...
        if fuzzy_match and similarity >= self.fuzzy_match_threshold:
            logger.info(f"Fuzzy match found: {fuzzy_match.name} (similarity: {similarity:.2f})")
            return MatchResult(
                product=fuzzy_match,
    ...
    def _find_alias_match(self, normalized_name: str) -> Tuple[Optional[Product], str]:
        ...
        try:
            products = Product.objects.filter(
                aliases__icontains=normalized_name,
                is_active=True
            )
            
            for product in products:
                for alias in product.aliases:
                    if self._calculate_similarity(normalized_name, alias.lower()) >= self.exact_match_threshold:
    ...
    def _find_fuzzy_match(self, normalized_name: str) -> Tuple[Optional[Product], float]:
        ...
        try:
            ...
            for product in products:
                ...
                name_similarity = self._calculate_similarity(normalized_name, product.name.lower())
    ...
    def _create_ghost_product(self, parsed_product: ParsedProduct, normalized_name: str) -> Product:
        ...
        try:
            ...
            logger.info(f"Created ghost product: {ghost_product.name} (category: {category})")
    ...
    def _guess_category(self, normalized_name: str) -> Optional[Category]:
        ...
        try:
            ...
            for category_name, keywords in self.category_keywords.items():
                ...
                matches = sum(1 for keyword in keywords if keyword in normalized_name.lower())
    ...
    def batch_match_products(self, parsed_products: List[ParsedProduct]) -> List[MatchResult]:
        """
    ...
    def update_product_aliases(self, product: Product, new_name: str) -> bool:
        ...
        try:
            ...
            if self._calculate_similarity(normalized_new_name, product.name.lower()) > 0.9:
                return False
            ...
            logger.info(f"Added alias '{normalized_new_name}' to product '{product.name}'")
            return True
