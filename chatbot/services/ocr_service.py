...
class OCRService:
    ...
    def __init__(self):
        ...
        self._primary_backend: OCRBackend | None = None
        self._fallback_backend: FallbackOCRBackend | None = None
    ...
    def process_file(
        ...
        if preferred_backend:
            ...
            backend = next(
                (b for b in self._backends if b.name == preferred_backend), None
            )
            if not backend:
        ...
        if result.success and result.text.strip():
            logger.info(
                f"OCR successful with {backend.name}: {result.confidence:.2f} confidence"
            )
            return result

        # Try fallback if enabled and available
        if use_fallback and self._fallback_backend and len(self._backends) > 1:
            logger.info("Primary backend failed, trying fallback")
            fallback_result = self._fallback_backend.process_file(file_path)

            if fallback_result.success:
                logger.info(
                    f"Fallback OCR successful: {fallback_result.confidence:.2f} confidence"
                )
    ...
    def extract_text_from_image(
        self, image_path: str, preferred_backend: str | None = None
    ) -> OCRResult:
        """Extract text from image file."""
        return self.process_file(image_path, preferred_backend=preferred_backend)

    def extract_text_from_pdf(
        self, pdf_path: str, preferred_backend: str | None = None
    ) -> OCRResult:
    ...
    def get_service_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "primary_backend": (
                self._primary_backend.name if self._primary_backend else None
            ),
