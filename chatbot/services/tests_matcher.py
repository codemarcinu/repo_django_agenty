...
from .product_matcher import MatchResult, ProductMatcher, get_product_matcher
...
class ProductMatcherTest(TestCase):
    ...
    def test_exact_match(self):
        ...
        parsed_product = ParsedProduct(name="Mleko 3,2%", total_price=Decimal("2.99"))
    ...
    def test_alias_match(self):
        ...
        parsed_product = ParsedProduct(name="milk", total_price=Decimal("2.99"))
    ...
    def test_fuzzy_match(self):
        ...
        parsed_product = ParsedProduct(
            name="Mleko Laciate 3.2%", total_price=Decimal("2.99")
        )

        result = self.matcher.match_product(parsed_product)

        self.assertIn(
            result.match_type, ["fuzzy", "exact"]
        )  # Could be either depending on similarity
        self.assertEqual(result.product, self.milk_product)
        self.assertGreater(result.confidence, 0.7)
    ...
    def test_ghost_product_creation(self):
        ...
        parsed_product = ParsedProduct(
            name="Unknown Product XYZ", total_price=Decimal("5.99")
        )
...
class ProductMatcherIntegrationTest(TestCase):
    ...
    def test_realistic_receipt_matching(self):
        ...
        receipt_products = [
            ...
            ParsedProduct(
                name="Jogurt nat. Danone 4x125g", total_price=Decimal("5.99")
            ),
            ParsedProduct(
                name="Banan kg", total_price=Decimal("6.80")
            ),  # Should create ghost
            ParsedProduct(
                name="MLEKO 2%", total_price=Decimal("3.29")
            ),  # Case variation
        ]
    ...
    def test_edge_cases(self):
        ...
        for result in results:
            self.assertIn(
                result.match_type, ["created", "exact", "fuzzy", "alias", "none"]
            )
    ...
    def test_performance_with_many_products(self):
        ...
        for i in range(100):
            Product.objects.create(
                name=f"Product {i}", aliases=[f"prod {i}", f"item {i}"]
            )
