...
class RegexReceiptParser(ReceiptParser):
    ...
    def parse(self, ocr_text: str) -> ParsedReceipt:
        ...
        try:
            ...
            logger.info(f"Parsed receipt: {len(receipt.products)} products, total: {receipt.total_amount}")
            return receipt
    ...
    def _extract_store_address(self, lines: List[str]) -> Optional[str]:
    ...
    def _extract_tax_id(self, lines: List[str]) -> Optional[str]:
    ...
    def _find_products_start(self, lines: List[str]) -> int:
        ...
        for i, line in enumerate(lines):
            ...
            if any(keyword in line_lower for keyword in [
                'nip', 'tax', 'adres', 'address', 'tel', 'phone', 'www', 'email'
            ]):
                continue
    ...
    def _find_products_end(self, lines: List[str]) -> int:
        ...
        for i, line in enumerate(lines):
            ...
            if any(keyword in line_lower for keyword in [
                'suma', 'razem', 'total', 'łącznie', 'lacznie',
                'do zapłaty', 'do zaplaty', 'payment', 'płatność', 'platnosc',
                'gotówka', 'gotowka', 'karta', 'card', 'reszta', 'change'
            ]):
                return i
    ...
    def _parse_product_line(self, line: str, line_number: int) -> Optional[ParsedProduct]:
    ...
    def _extract_simple_product(self, line: str, line_number: int) -> Optional[ParsedProduct]:
        ...
        name = re.sub(r'\s+\d+(?:[,.]?\d+)?\s*(?:kg|g|l|ml|szt|pcs)?\s*$', '', name, flags=re.IGNORECASE)
        name = name.strip()
