...
def orchestrate_receipt_processing(self, receipt_id: int):
    ...
    # WSZYSTKIE serwisy są importowane tutaj. To jest klucz do rozwiązania problemu.
    @shared_task(bind=True, max_retries=3, default_retry_delay=60)
def orchestrate_receipt_processing(self, receipt_id: int):
    """
    Główne zadanie Celery orkiestrujące cały proces przetwarzania paragonu.
    ...
    from chatbot.schemas import OCRResult as SchemaOCRResult # Import schematu dla Quality Gate
    ...
    try:
        ...
        # Poprawka: Zapis do poprawnego pola 'raw_text'
        ...
        # Poprawka: Tłumaczenie obiektu OCR na schemat dla QualityGateService
        quality_gate_input = SchemaOCRResult(
            full_text=ocr_result.text,
            lines=ocr_result.metadata.get('lines', []),
            confidences=ocr_result.metadata.get('confidences', [])
        )
        quality_gate = QualityGateService(quality_gate_input)
        ...
        if quality_score >= settings.OCR_QUALITY_THRESHOLD:
            ...
            raw_text = ocr_result.text # Poprawka: Użycie .text zamiast .get_full_text()
        ...
        raise self.retry(exc=e) # Importujemy instancję, nie klasę
    from chatbot.services.quality_gate_service import QualityGateService
    from chatbot.services.registry import receipt_parser
    from chatbot.services.vision_service import VisionService
    from chatbot.services.product_matcher import ProductMatcher
