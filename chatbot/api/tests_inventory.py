...
from inventory.models import Category, ConsumptionEvent, InventoryItem, Product
...
class ConsumeInventoryAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name="Test Product", category=self.category, reorder_point=Decimal("2.000")
        )
    ...
    def test_successful_consumption(self):
        ...
        url = reverse(
            "chatbot_api:inventory-consume",
            kwargs={"inventory_id": self.inventory_item.id},
        )
        data = {"consumed_qty": "2.5", "notes": "Test consumption"}
        ...
        consumption = ConsumptionEvent.objects.filter(
    ...
    def test_consumption_insufficient_quantity(self):
        ...
        data = {"consumed_qty": "10.0"}  # More than available (5.0)
    ...
    def test_consumption_invalid_quantity(self):
        ...
        # Test zero quantity
        response = self.client.post(
            url, data=json.dumps({"consumed_qty": "0"}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertFalse(response.json()["success"])
    ...
    def test_consumption_missing_quantity(self):
        ...
        data = {"notes": "Test without quantity"}
...
class InventoryAlertsTaskTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(
            name="Perishable Category", meta={"default_expiry_days": 7}
        )
...
class InventoryIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(
            name="Integration Test Category", meta={"default_expiry_days": 5}
        )
    ...
    def test_consumption_triggers_low_stock_alert(self):
        ...
        self.assertEqual(
            len(item_alerts), 1
        )  # Only one alert per item due to deduplication
