        ...
        for item in low_stock_items:
            ...
            if not any(alert['item_id'] == item.id for alert in alerts):
                alerts.append({
                    ...
                    'unit': item.unit,
                    'storage_location': item.storage_location,
                    'reorder_point': float(item.product.reorder_point) if item.product.reorder_point else 0,
                    'expiry_date': item.expiry_date.isoformat() if item.expiry_date else None,
                })
...
def send_inventory_alerts_notification(self, alerts: List[Dict[str, Any]]):
    ...
    try:
        ...
        # Send email
        recipient_email = getattr(settings, 'INVENTORY_ALERTS_EMAIL', 'admin@example.com')
