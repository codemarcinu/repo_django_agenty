...
class ReceiptProcessor:
    ...
    async def _extract_products_with_llm(self, receipt_text):
        ...
        ollama_agent = OllamaAgent(
            config={"model": "SpeakLeash/bielik-11b-v2.3-instruct:Q5_K_M"}
        ...
        try:
            ...
            if response.success:
                ...
                if json_start != -1 and json_end != -1 and json_end > json_start:
                    ...
                    logger.info(
                        f"✅ Successfully parsed {len(products_data)} products from LLM response"
                    )
    ...
    async def process_receipt(self, receipt_processing_id):
        logger.info(
            f"🔄 STARTING OCR PROCESSING for receipt ID: {receipt_processing_id}"
        )
        receipt_record = None
        ...
        try:
            ...
            logger.info(
                f"Updating receipt {receipt_processing_id} status to 'ocr_in_progress'"
            )
            ...
            logger.info(
                f"🤖 Starting LLM product extraction for receipt {receipt_processing_id}"
            )
            receipt_record.status = "llm_in_progress"
            await sync_to_async(receipt_record.save)()
            logger.debug(
                f"Receipt {receipt_processing_id} status updated to 'llm_in_progress'"
            )
            ...
            logger.info(
                f"🎉 Receipt {receipt_processing_id} processing COMPLETED and ready for review!"
            )
            ...
            logger.error(
                f"❌ ReceiptProcessing record with ID {receipt_processing_id} not found in database"
            )
            return False
        except Exception as e:
            logger.error(
                f"❌ CRITICAL ERROR during receipt processing for ID {receipt_processing_id}: {e}",
                exc_info=True,
            )
            if receipt_record:
                try:
                    ...
                    await sync_to_async(receipt_record.save)()
                    logger.info(
                        f"Marked receipt {receipt_processing_id} as error in database"
                    )
                except Exception as save_error:
                    logger.error(
                        f"Failed to save error status for receipt {receipt_processing_id}: {save_error}"
                    )
            return False
