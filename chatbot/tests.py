...
class ReceiptProcessingTasksTest(TestCase):
    ...
    def setUp(self):
        ...
        self.receipt_record = ReceiptProcessing.objects.create(
            receipt_file=self.dummy_file,
            status='uploaded'
        )

    @patch('chatbot.tasks.receipt_processor')
    @patch('chatbot.tasks.easyocr') # Mock easyocr if it's used directly in receipt_processor
    def test_process_receipt_task_success(self, mock_easyocr, mock_receipt_processor):
        # Configure mocks
        mock_receipt_processor.process_receipt.return_value = None # Assuming it doesn't return anything specific
        mock_easyocr.Reader.return_value.readtext.return_value = [('text', (0,0,0,0), 0.9)] # Example OCR output
        ...
        # Assertions
        mock_receipt_processor.process_receipt.assert_called_once_with(self.receipt_record.id)
        self.assertEqual(self.receipt_record.status, 'completed') # Assuming task sets status to completed on success
