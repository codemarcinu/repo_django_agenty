...
from .models import Agent, Conversation, Message, Document
...
class AgentAdmin(admin.ModelAdmin):
    list_display = ('name', 'agent_type', 'is_active', 'created_at', 'capabilities_count')
...
class ConversationAdmin(admin.ModelAdmin):
    list_display = ('title_display', 'agent', 'session_id_short', 'user_id', 'is_active', 'message_count', 'created_at')
    ...
    fieldsets = (
        ...
        ('Szczegóły', {
            'fields': ('session_id', 'summary', 'metadata')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
